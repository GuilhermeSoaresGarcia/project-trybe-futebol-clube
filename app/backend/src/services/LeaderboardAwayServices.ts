...
export default class LeaderboardAwayServices {
  ...
  private static goalStats = async () => {
    ...
    const result = allTeams.map((team) => {
      const goalsFavor = team.awayTeam?.reduce((acc, curr) =>
        acc + curr.awayTeamGoals, 0) as number;
      const goalsOwn = team.awayTeam?.reduce((acc, curr) =>
  ...
  private static teamStats = async () => {
    const allTeams = await this.getTeamData();
    const result = allTeams.map((team) => {
      const totalVictories = team.awayTeam?.filter(
        (match: Match) => (match.awayTeamGoals > match.homeTeamGoals),
      ).length as number;
      const totalDraws = team.awayTeam?.filter((match: Match) =>
        match.awayTeamGoals === match.homeTeamGoals).length;
      const totalPoints = (totalVictories as number) * 3 + (totalDraws as number);
      const totalLosses = team.awayTeam?.filter((match: Match) =>
        match.awayTeamGoals < match.homeTeamGoals).length;
      const totalGames = team.awayTeam?.length as number;
